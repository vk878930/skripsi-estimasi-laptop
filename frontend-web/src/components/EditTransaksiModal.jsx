        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-row">
            <div className="input-group">
              ...
              <input type="text" value={header.order_id} onChange={(e) => setHeader({...header, order_id: e.target.value})} required readOnly style={{backgroundColor: '#e2e8f0'}} title="Order ID tidak bisa diubah" />
            </div>
            <div className="input-group">
          ...
          {items.map((item, index) => (
            <div key={index} className="item-input-group">
              <div className="form-row" style={{ marginBottom: '10px' }}>
                ...
                <input type="text" name="merek" placeholder="Merek" value={item.merek} onChange={(e) => handleItemChange(index, e)} required style={{ flex: 1 }}/>
                <input type="text" name="nama_unit" placeholder="Unit" value={item.nama_unit} onChange={(e) => handleItemChange(index, e)} required style={{ flex: 1.5 }}/>
                <input type="text" name="processor" placeholder="Processor" value={item.processor} onChange={(e) => handleItemChange(index, e)} required style={{ flex: 1 }}/>
                {items.length > 1 && (
              ...
              <div className="form-row" style={{ marginBottom: 0 }}>
                <input type="number" name="ram" placeholder="RAM" value={item.ram === 0 ? '' : item.ram} onChange={(e) => handleItemChange(index, e)} required style={{ width: '80px' }}/>
                <input type="number" name="ssd" placeholder="SSD" value={item.ssd === 0 ? '' : item.ssd} onChange={(e) => handleItemChange(index, e)} required style={{ width: '80px' }}/>
                <input type="number" name="tahun" placeholder="Tahun" value={item.tahun === 0 ? '' : item.tahun} onChange={(e) => handleItemChange(index, e)} required style={{ width: '80px' }}/>
                ...
                <input type="number" name="harga_terjual" placeholder="Harga" value={item.harga_terjual === 0 ? '' : item.harga_terjual} onChange={(e) => handleItemChange(index, e)} required style={{ flex: 1 }}/>
          ...
          <div className="modal-actions">
            ...
            <button type="submit" className="btn-primary" style={{ width: 'auto', backgroundColor: '#eab308' }}>Simpan Perubahan</button>
          </div>
        </form>
      </div>
    </div>
